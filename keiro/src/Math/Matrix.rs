    ...
    T: Mul<Output = T> + Add<Output = T> + Copy,
{
    ...
    pub fn GetColumn(&self, index: usize) -> Vector4<T> {
        ...
        Vector4::new(
            ...
            self.content[2][index],
            self.content[3][index],
        )
    }
...
impl<T> Matrix4<T>
where
    T: From<f32> + Mul<Output = T> + Add<Output = T> + Sub<Output = T> + Copy,
{
    ...
    pub fn Frustum(fovy: f32, aspect: f32, near: f32, far: f32) -> Matrix4<T> {
        ...
        Matrix4::new(
            ...
            T::from(0.0),
            T::from(0.0),
            T::from((far + near) / (far - near)),
            T::from(-(2.0 * far * near) / (far - near)),
            T::from(0.0),
            T::from(0.0),
            T::from(1.0),
            T::from(0.0),
